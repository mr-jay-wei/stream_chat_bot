...
# from .user_service import UserService  # 暂时注释掉，因为UserService有问题
...
class MemoryManager:
    ...
    def add_message_to_session(self, db: Session, session_id: int, role: str, content: str) -> None:
        """向对话会话添加消息"""
        try:
            # 检查消息长度
            ...
            # 暂时注释掉，因为UserService的方法被注释了
            # UserService.add_message_to_session(db, session_id, role, content)
            logger.debug(f"向会话 {session_id} 添加 {role} 消息")
            
        except Exception as e:
            logger.error(f"添加消息失败: {e}")
    
    def get_session_context(self, db: Session, session_id: int, user_id: int, max_turns: Optional[int] = None) -> List[ConversationTurn]:
        """获取对话会话的上下文（转换为ConversationTurn格式以兼容现有代码）"""
        try:
            # 暂时注释掉，因为UserService的方法被注释了
            # messages = UserService.get_session_messages(db, session_id, user_id)
    ...
    def create_new_session(self, db: Session, user_id: int, first_question: str) -> Optional[int]:
        ...
        try:
            ...
            # session = UserService.create_chat_session(db, user_id, title)
            # if session:
            #     return session.id
            return None
    ...
    def get_memory_stats(self, db: Session, user_id: int) -> Dict[str, Any]:
        ...
        try:
            # 暂时注释掉，因为UserService的方法被注释了
            # sessions = UserService.get_user_chat_sessions(db, user_id)
